    ...
    def wttr( location, forecast, metric, language, colors, one_line):
        ...
        try:
            ...
            if response.ok:
                print(response.text)
                data = response.text
                __class__.result= data
                #:Workaround
                obj = weather()
                obj.speek_Temp()
            else:
                print(weather.ERR_MSG.format(response.status_code))
        except requests.exceptions.ConnectionError:
            print(weather.CONN_ERR)

    def speek_Temp(self):
        mydict = []
        mydict = __class__.result
        indexOfComma = mydict.index(',')
        _city =mydict[:indexOfComma]
        indexOfC = mydict.index('C')
        indexOfPlus = mydict.index('+')
        _temp= mydict[indexOfPlus+1:indexOfC-1]
        obj= _TTS()
        obj.speak(f"You are in {_city} city, and temperature is {_temp} Degree celsius")
